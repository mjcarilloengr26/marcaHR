...
router.get("/", requireAuth, (req, res) => {
  ...
  if (!isHr && !isSalesEmployee(req)) return res.status(403).json({ error: "Insufficient permissions" });
...
router.put("/:id", requireAuth, (req, res) => {
  ...
  const isHr = ["admin", "hr"].includes(req.user.role);
  if (!isHr) {
    if (!isSalesEmployee(req) || existing.owner_id !== req.user.employee_id) {
...
router.delete("/:id", requireAuth, (req, res) => {
  ...
  const isHr = ["admin", "hr"].includes(req.user.role);
  if (!isHr && (!isSalesEmployee(req) || existing.owner_id !== req.user.employee_id)) {
    return res.status(403).json({ error: "Insufficient permissions" });
  }
